...
# arango outputs
api_stats_outputs = OutputRegistry('c7n.arango_output.api_stats')
blob_outputs = BlobOutputRegistry('c7n.arango_output.blob')
log_outputs = LogOutputRegistry('c7n.arango_output.logs')
...
class Metrics:
    ...
    def __init__(self, ctx, config=None):
        ...
        try:
            self._connect()
            self.db.AQLQuery(self.init_query, bindVars={
    ...
    def flush(self):
        if self.buf:
            self.write_to_db(self.namespace, self.buf)
            self.buf = []
...
class BlobOutput(DirectoryOutput):
    ...
    log = logging.getLogger('custodian.arango_output.blob')
